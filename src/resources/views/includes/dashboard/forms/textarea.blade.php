    ...
    @foreach ($form->translatables() as $translatable)
        <div class="col-md-{{ $form->column() }} {{ $form->className() }}" data-label="container_{{ $key }}">
            <div class="form-group">
                ...
                @include('adminro::includes.dashboard.forms.utils.textarea', ['key' => $key, 'suffix' => getTranslatableKeySuffix($translatable), 'form' => $form->attributes(), 'model' => $controllerSettings->model()->model(), 'edit_mode' => $controllerSettings->request()->editMode(), 'translatable' => $translatable])
                @error($key)
...
@else
    <div class="col-md-{{ $form->column() }} {{ $form->className() }}" data-label="container_{{ $key }}">
        <div class="form-group">
            ...
            @include('adminro::includes.dashboard.forms.utils.textarea', ['key' => $key, 'form' => $form->attributes(), 'model' => $controllerSettings->model()->model(), 'edit_mode' => $controllerSettings->request()->editMode()])
            @error($key)
                <span class="form-text text-danger font-weight-bold">{{ $message }}</span>
            @endError
        </div>
