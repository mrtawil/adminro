        ...
        <div class="form-group">
            ...
            <select name="{{ $key }}[]" id="{{ $key }}" class="searchable" @if ($form->multiple()) multiple="multiple" @endif>
                @foreach ($multiselect->items() as $item)
                    <option value="{{ $multiselect->value($item) }}" @if (!$form->hiddenValue()) @if ($controllerSettings->request()->editMode() && in_array($multiselect->value($item), $multiselect->itemsSelected(), true)) selected @elseif($controllerSettings->request()->editMode() && in_array(strval($multiselect->value($item)), is_array(old($key)) ? old($key) : [strval(old($key))], true)) selected @endif @endif>
