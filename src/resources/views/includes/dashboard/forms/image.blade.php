            ...
            <div class="d-flex align-items-center">
                ...
                @if ($controllerSettings->request()->editMode() && !$form->hiddenValue() && isset($controllerSettings->model()->model()[$key]) && $controllerSettings->model()->model()[$key])
                    <div class="dropzone-panel mb-lg-0 mb-2">
                        <a class="dropzone-select btn btn-light-danger font-weight-bold btn-sm" href="{{ $controllerSettings->info()->removeFileUrl($key) }}">Remove image</a>
                    </div>
                @endif
            </div>
            <div class="row">
        ...
        @if ($controllerSettings->request()->editMode() && !$form->hiddenValue() && isset($controllerSettings->model()->model()[$key]) && $controllerSettings->model()->model()[$key])
            <div class="image-input image-input-outline mt-2">
                <div class="image-input-wrapper">
                    <img src="{{ getStorageUrl($controllerSettings->model()->model()[$key . '_path'],$controllerSettings->model()->model()[$key]) }}" class="img-thumbnail" alt="">
                </div>
            </div>
        @endif
        @error($key)
