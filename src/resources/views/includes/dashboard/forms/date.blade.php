<div class="col-md-{{ $form->column() }} {{ $form->className() }}" data-label="container_{{ $key }}">
    <div class="form-group">
        ...
        <div class="input-group">
            <input type="text" name="{{ $key }}" id="{{ $key }}" class="form-control form-control-lg form-control-solid" autocomplete="off" placeholder="{{ $form->placeholder() }}" max="{{ $form->maxValue() }}" min="{{ $form->minValue() }}" step="{{ $form->step() }}" maxlength="{{ $form->maxLength() }}" @if (($controllerSettings->request()->editMode() && $form->requiredEdit()) || (!$controllerSettings->request()->editMode() && $form->requiredCreate())) required @endif @if (!$form->hiddenValue()) @if ($controllerSettings->request()->editMode()) @isset($controllerSettings->model()->model()[$key]) @if ($controllerSettings->model()->model()[$key] instanceof \Carbon\Carbon) value="{{ $controllerSettings->model()->model()[$key]->toDateString() }}" @else value="{{ \Carbon\Carbon::parse($controllerSettings->model()->model()[$key])->toDateString() }}" @endif @endisset @else value="@if (old($key)){{ old($key) }}@else{{ $form->valueCreate() }}@endif" @endif @endif @if (($controllerSettings->request()->editMode() && $form->readOnlyEdit()) || (!$controllerSettings->request()->editMode() && $form->readOnlyCreate())) readonly @endif @if (($controllerSettings->request()->editMode() && $form->disabledEdit()) || (!$controllerSettings->request()->editMode() && $form->disabledCreate())) disabled @endif />
