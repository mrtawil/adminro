    ...
    @if (config('dashboard'))
        <div class="aside-menu-wrapper flex-column-fluid" id="kt_aside_menu_wrapper">
            <div id="kt_aside_menu" class="aside-menu my-4" data-menu-vertical="1" data-menu-scroll="1" data-menu-dropdown-timeout="500">
                <ul class="menu-nav">
                    @foreach (config('dashboard.aside.sections') as $section)
                        @can($section['permission'])
                            ...
                            @endif
                            @if (isset($section['type']) && $section['type'] == 'function')
                                @php
                                    $section['parents'] = $section['function']();
                                @endphp
                            @endif
                                        ...
                                        @if ($parent['intent'])
                                            <div class="menu-submenu "><i class="menu-arrow"></i>
                                                <ul class="menu-subnav">
                                                    <li class="menu-item menu-item-parent" aria-haspopup="true"><span class="menu-link"><span class="menu-text">{{ $parent['name'] }}</span></span></li>
                                                    @if ($parent['directlyItems'])
                                                        <ul class="menu-subnav">
                                                            <li class="menu-item menu-item-parent" aria-haspopup="true"><span class="menu-link"><span class="menu-text">{{ $parent['name'] }}</span></span></li>
                                                            @foreach ($parent['items'] as $item)
                                                                @can($item['permission'])
                                                                    <li @if ($item['tooltip']) data-toggle="tooltip" data-placement="left" data-theme="dark" @endif title="{{ $item['title'] }}" class="menu-item @if (isCurrentUrl($controllerSettings->info()->activeRoute(), 'item', $item['id'], $item['href'])) menu-item-active @endif" aria-haspopup="true">
                                                                        <a @if (($item['type'] == 'asset' || $item['type'] == 'route') && $item['href']) href="{{ route($item['href']) }}" @else href="javascript:;" @endif class="menu-link ">
