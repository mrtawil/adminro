<div class="col-md-{{ $form['column'] }} {{ $form['class_name'] }}" data-label="container_{{ $key }}">
    <div class="form-group">
        ...
        <div class="dropzone dropzone-multi" id="dropzone-{{ $key }}">
            <div class="d-flex align-items-center">
                <div class="dropzone-panel mb-lg-0 mb-2 mr-2">
                    <a class="dropzone-select btn btn-light-primary font-weight-bold btn-sm" onclick="onAttachFilesClick('{{ $key }}')">Attach file</a>
                </div>
                @if ($edit_mode && !$form['hidden_value'] && isset($model[$key]) && $model[$key])
                    <div class="dropzone-panel mb-lg-0 mb-2">
                        <a class="dropzone-select btn btn-light-danger font-weight-bold btn-sm" onclick="return confirm('Are you sure?') || event.stopImmediatePropagation();" wire:click.prevent="removeFile">Remove file</a>
                    </div>
                @endif
            </div>
            <div class="row">
        ...
        @if ($edit_mode && !$form['hidden_value'] && isset($model[$key]) && $model[$key])
            <div class="mt-2">
                <a href="{{ getStorageUrl($model[$key . '_path'], $model[$key]) }}" class="btn btn-light text-secondary font-weight-bold btn-sm mr-1" target="_blank">Preview</a>
                <a href="{{ getStorageUrl($model[$key . '_path'], $model[$key]) }}" class="btn btn-light text-secondary font-weight-bold btn-sm" target="_blank" download>Download</a>
            </div>
        @endif
        @error($key)
